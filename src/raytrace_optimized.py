...
Change vs. original (bm_raytrace/run_benchmark.py):
- Added `__slots__` to every hand-rolled class (Vector, Point, Sphere,
  Halfspace, Ray, Scene, SimpleSurface, CheckerboardSurface, Canvas).

Why: none of these classes used __slots__, so every instance carried a
...
attribute-heavy classes like these.

Everything else (algorithm, scene, shading model) is unchanged from the
original, to keep this a fair, isolated comparison.
"""
...
if __name__ == "__main__":
    ...
    runner.metadata['description'] = "Simple raytracer (optimized: __slots__)"
